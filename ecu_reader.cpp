...
static const QBluetoothUuid SPP_UUID("{00001101-0000-1000-8000-00805F9B34FB}"); // RFCOMM SPP
    ...
    : QObject(parent)
{
    ...
    connect(m_agent.get(), &QBluetoothDeviceDiscoveryAgent::finished,
            this, &EcuReader::onScanFinished);
    connect(m_agent.get(), &QBluetoothDeviceDiscoveryAgent::canceled,
            this, &EcuReader::onScanFinished);
#if QT_VERSION >= QT_VERSION_CHECK(6, 4, 0)
    connect(m_agent.get(), &QBluetoothDeviceDiscoveryAgent::errorOccurred,
            this, &EcuReader::onScanError);
#else
    connect(m_agent.get(), qOverload<QBluetoothDeviceDiscoveryAgent::Error>(&QBluetoothDeviceDiscoveryAgent::error),
            this, &EcuReader::onScanError);
#endif
}
...
void EcuReader::connectToDevice() {
    if (!m_address.isNull()) {
        ...
        connect(m_socket.get(), &QBluetoothSocket::readyRead, this, &EcuReader::onReadyRead);
        connect(m_socket.get(), &QBluetoothSocket::stateChanged, this, &EcuReader::onStateChanged);
        connect(m_socket.get(), &QBluetoothSocket::errorOccurred, this, &EcuReader::onErrorOccurred);
...
bool EcuReader::loadXmlMap(const QString& urlOrPath) {
    ...
    while (!xr.atEnd()) {
        ...
        if (xr.isStartElement() && xr.name() == QLatin1String("symbol")) {
            ...
            if (!a.hasAttribute("channel")) continue;
            ...
            ci.divider = a.hasAttribute("divider") ? a.value("divider").toString().toDouble() : 1.0;
            ci.offset  = a.hasAttribute("offset")  ? a.value("offset").toString().toDouble()  : 0.0;
...
void EcuReader::startScan() {
    if (QOperatingSystemVersion::currentType() == QOperatingSystemVersion::Windows) {
        m_lastError = "Windows: please pair ECUMaster in OS settings; app-side scan is limited.";
        emit errorChanged(m_lastError);
    ...
    if (m_local.isValid() && m_local.hostMode() == QBluetoothLocalDevice::HostPoweredOff)
        m_local.powerOn();
...
void EcuReader::onDeviceFound(const QBluetoothDeviceInfo& info) {
    ...
    m_found.append(info);
    const QString line = QString("%1 (%2)").arg(info.name(), info.address().toString());
...
void EcuReader::onScanFinished() {
    m_scanning = false;
    emit scanningChanged();
    emit info(QString("Bluetooth scan finished. Found %1 device(s).").arg(m_found.size()));
}
...
void EcuReader::connectToName(const QString& nameOrSubstring) {
    ...
    static const QRegularExpression macRx(QStringLiteral("^[0-9A-Fa-f]{2}(:[0-9A-Fa-f]{2}){5}$"));
    ...
    for (auto it = m_found.cbegin(); it != m_found.cend(); ++it) {
        ...
        if (nm.compare(nameOrSubstring, Qt::CaseInsensitive) == 0) { itPick = it; break; }
        if (nm.contains(nameOrSubstring, Qt::CaseInsensitive) && itPick == m_found.cend())
            itPick = it;
    }
...
void EcuReader::onErrorOccurred(QBluetoothSocket::SocketError) {
    ...
    emit errorChanged(m_lastError);          // (or emit errorChanged(m_lastError, code) if you pass codes)
    if (!isConnected()) setConnected(false); // keep property/signals consistent
    emit warn(m_lastError);
}
...
void EcuReader::parseIncoming() {
    int ch; quint8 vh, vl, cs;
    while (tryExtractFrame(ch, vh, vl, cs)) {
        const ChannelInfo info = m_chmap.value(ch, ChannelInfo{QString(), "word", 1.0, 0.0, QString()});
...
bool EcuReader::tryExtractFrame(int& ch, quint8& vh, quint8& vl, quint8& cs) {
    ...
    for (int i = 0; i + 5 <= m_buf.size(); ++i) {
        ...
        if (B1 != ID_CHAR) continue;
        const bool ok256 = checksumOk(B0, B2, B3, B4, 256);
        const bool ok255 = !ok256 && checksumOk(B0, B2, B3, B4, 255);
        if (!ok256 && !ok255) continue;
        ch = int(B0); vh = B2; vl = B3; cs = B4;
        m_buf.remove(0, i+5);
        return true;
    }
    // avoid unbounded growth on garbage
    if (m_buf.size() > 4096) m_buf.remove(0, m_buf.size() - 1024);
    return false;
}

bool EcuReader::checksumOk(quint8 ch, quint8 vh, quint8 vl, quint8 cs, int mod) {
...
qint32 EcuReader::decodeRaw(const QString& storage, quint8 vh, quint8 vl) {
    ...
    if (s=="word" || s=="sword") {
        ...
        if (s=="sword") return (u & 0x8000) ? (qint32(u) - 0x10000) : qint32(u);
        return qint32(u);
    }
    qint32 u8 = vl;                 // 8-bit signals in low byte
    if (s=="sbyte") return (u8 & 0x80) ? (u8 - 0x100) : u8;
    return u8;                       // ubyte/percent7
}

double EcuReader::scaleValue(const ChannelInfo& info, qint32 raw) {
    double v = raw;
    if (info.divider != 0.0) v /= info.divider;
    v += info.offset;
    return v;
}
...
void EcuReader::applyChannel(int ch, double v) {
    ...
    if (nm.contains("baro") || nm.contains("atmo")) {
        ...
        if (nv != m_baro) { m_baro = nv; emit baroChanged(); }
    }
    // v1.218 key channels
    switch (ch) {
    case 1:  { int nv = int(qRound(v));        if (nv!=m_rpm)    { m_rpm=nv;    emit rpmChanged();    } break; } // RPM word/1
    case 2:  { int nv = int(qRound(v));        if (nv!=m_map)    { m_map=nv;    emit mapChanged();    } break; } // MAP kPa word/1
    case 3:  { int nv = int(qRound(v));        if (nv!=m_tps)    { m_tps=nv;    emit tpsChanged();    } break; } // TPS % ubyte/1
    case 4:  { int nv = int(qRound(v));        if (nv!=m_iat)    { m_iat=nv;    emit iatChanged();    } break; } // IAT C sbyte/1
    case 5:  { double nv = v;                  if (qFabs(nv-m_batt) > 0.01) { m_batt=nv; emit battChanged(); } break; } // Batt word/37 V
    case 12: { double nv = v;                  if (qFabs(nv-m_afr)  > 0.01) { m_afr =nv; emit afrChanged();  } break; } // AFR ubyte/10
    case 24: { int nv = int(qRound(v));        if (nv!=m_clt)    { m_clt=nv;    emit cltChanged();    } break; } // CLT sword/1 C
    case 27: { double nv = v;                  if (qFabs(nv-m_lambda) > 0.001) { m_lambda=nv; emit lambdaChanged(); } break; } // λ ubyte/128
    default: break;
    }
}
