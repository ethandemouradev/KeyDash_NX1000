#include "crashlog.h"  // <— match the actual filename casing
...
std::mutex  g_mutex;
QString     g_logPath;
QString     g_currentBaseName = QStringLiteral("crashlog"); // base for .txt and (on Win) .dmp

// ---------- paths & rotation (cross-platform) ----------
QString logRootFor(const QString& appName) {
    // e.g. C:\Users\<you>\AppData\Local\<Org>\<App>
...
QString rotateIfExists(const QString& dir, const QString& baseName, const QString& ext) {
    ...
    const QString archived = QDir(dir).filePath(
        QStringLiteral("%1-%2.%3").arg(baseName, timestampNow(), ext));
...
#ifdef Q_OS_WIN
// ---------- Windows minidump support (dynamic DbgHelp load; no link needed) ----------
#ifndef NOMINMAX
#define NOMINMAX
#endif
#include <windows.h>
#include <DbgHelp.h> // for types only
...
static LONG WINAPI unhandledExceptionFilter(EXCEPTION_POINTERS* ex) {
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
        ...
        WriteMiniDumpDynamic(GetCurrentProcess(), GetCurrentProcessId(),
                             hFile,
...
void init(const QString& appName, const QString& version) {
    const QString app = appName.isEmpty() ? QCoreApplication::applicationName() : appName;
    ...
    g_ts << "App: "      << app << "\n";
    g_ts << "Version: "  << (version.isEmpty() ? QCoreApplication::applicationVersion() : version) << "\n";
    g_ts << "Qt: "       << QT_VERSION_STR << "\n";
    g_ts << "OS: "       << QSysInfo::prettyProductName() << " (" << QSysInfo::currentCpuArchitecture() << ")\n";
